...
\[
  \SSS_S = \sum_\Phi \mu_\Phi L_\Phi = \sum_{(\Phi_\text{coarse};\Phi_\text{fine})} \mu_{(\Phi_\text{coarse};\Phi_\text{fine})} L_{(\Phi_\text{coarse};\Phi_\text{fine})}
\]
be the unique representation of $\SSS_S$ in the fundamental basis of nc-quasisymmetric functions and a representation of $\SSS_S$ in the fundamental generating system of nc-quasisymmetric functions, respectively. Moreover, let $Q_S$ denote the scheduling quasisymmetric function and let
\[
  Q_S = \sum_{\alpha} \lambda_\alpha L_\alpha
\]
be its representation in terms of the fundamental basis of quasisymmetric functions. Then
\[
...
\]
for $i=1,\ldots,n$. In particular $(0,h^*_1,\ldots,h^*_n)$ is the $h^*$-vector of the polynomial $\xi_S(k)$. The coefficients $h_i^*$, $\mu_\Phi$ and $\mu_{(\Phi_\text{coarse};\Phi_\text{fine})}$ are integral but may be negative.
\end{theorem}

\comment{Todo: revise this proof.}
...
\begin{itemize}
\item Revise proof.
\item Integrate with preliminaries.
