...
I think section 5.1 needs to be tightened up quite a bit...also make more explicit the contributions of Thm 5.1, in particular that we are showing that counting function is a polynomial}
...
${P}_G$ as subdivided by the Coxeter comple~\cite{SW}.

The construction above was extended from graphical zonotopes to 
generalized permutahedron in~\cite{ABK}.  Generalized permutahedron
are typically defined as the class of polytopes in $\mathbb{R}^n$ such
that every edge is parallel to $e_i - e_j$ for some $i,j \in [n]$.  An
equivalent characterization of this class is the following: A polytope
is a generalized permutahedron if and only if its normal fan is
refined by the Braid arrangement~\cite{PRW, MPSSW}.  Hence the
...
$S$ can be associated to any generalized permutahedron $GP$ by defining
the forbidden configuraiton $\forb(S)$ to be the codimension one
skeleton of the normal fan of $GP$ as subdivided by the Coxeter
...
integer points in the interior of the normal fan

In~\cite{SW}, this perspective is used to prove that the coloring
complex has a convex ear decomposition which implies strong relations on
the chromatic polynomial.  Due to the close relation of the chromatic
polynomial of a graph, the authors attempt to generalize their results
to characteristic polynomials of matroids.  They note empiracally
...
matroid polytope.  The corresponding scheduling polynomial is the
...
natural and does yield good behavior.
   ...
   co-nc-fundamental basis.  This basis was introduced in~\cite{Zab} and is precisely opposite the directed refinement relation defined for the findamental basis.   Namely,
 $$\ncN_{\Phi} := \sum_{\Psi:\Phi \preceq \Psi} \ncM_{\Phi}.$$
 Again, after applying the type map to the indexing ordered set partitions, we arrive at a quasisymmetric expansion in terms of all compositions that coarsen a fixed composition.  Namely, if 

\comment{Displayed equation of N written in terms of M for QSym.  Comment about antipode?}
