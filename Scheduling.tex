...
\[
  h^*_i = \sum_{\Phi: \operatorname{length}(\Phi) = i} \lambda_\Phi =
   ...
   }} \lambda_{(\Phi_\text{coarse};\Phi_\text{fine})}
\]
for $i=1,\ldots,n$. In particular $(0,h^*_1,\ldots,h^*_n)$ is the $h^*$-vector of the polynomial $\xi_S(k)$. The coefficients $h_i^*$, $\lambda_\Phi$ and $\lambda_{(\Phi_\text{coarse};\Phi_\text{fine})}$ are integral but may be negative.
\end{theorem}
...
\]
where $i$ is the length of $\alpha$.
...
\[
  \xi_S(k) = \sum_{i=1}^{n} h^*_i \binom{k+n-i}{n} \;\;\; \text{ where } \;\;\; h^*_i = \sum_{\alpha: \operatorname{length}(\alpha)=i} \lambda_\alpha.
\]
In particular, the vector $(0,h^*_1,\ldots,h^*_n)$ is the $h^*$-vector of the polynomial $\xi_S$. The coefficients $h^*_i$ and $\lambda_\alpha$ are integral but can be negative.
\end{theorem}

\begin{proof}
We have $Q_S(\mathbf{1}^k)=\xi_S(k)$. Moreover, we have seen that $L_\alpha(\mathbf{1}^k)=\binom{k+n-i}{n}$ where $i$ denotes the length of $\alpha$. The length of an ordered partition of $n$ can range from $1$ to $n$. By collecting terms, we obtain the theorem. Note that the coefficients in the fundamental basis are integral because the coefficients in the monomial basis are integral (in fact, binary) for any scheduling problem.
\end{proof}
