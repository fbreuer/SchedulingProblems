...
\newcommand{\TS}{\T_{S^{n-1}}} % the braid triangulation of the sphere (i.e. the Coxeter complex)
...
\newcommand{\allowS}{\allow_{S^{n-1}}} % the braid triangulation of the sphere (i.e. the Coxeter complex)
...
\newcommand{\forbS}{\forb_{S^{n-1}}} % the braid triangulation of the sphere (i.e. the Coxeter complex)
...
\comment{TODO: Insert remarks from below about the connections of these objects to the literature here.}
...
\] 
However, these coefficients $h_i$ are not uniquely determined as there is one degree of freedom too many. Two conventions for normalization exist in the literature. The first convention is to fix $h_0=1$, which gives rise to the $h$-vector as commonly used in the study of simplicial complexes. The second convention is to fix $h_{d+1}=0$, which gives rise to the $h^*$-vector used in Ehrhart theory. Formally, given numbers $f_0,\ldots,f_d$, the \emph{$h$-vector} $h_0,\ldots,h_{d+1}$ and the \emph{$h^*$-vector} are defined, respectively, via
\[
  \binom{k+d}{d} + \sum_{i=1}^{d+1} h_i \binom{k+d-i}{d} = \sum_{i=0}^{d} f_i \binom{k-1}{i} = \sum_{i=0}^{d} h^*_i \binom{k+d-i}{d},
\]
and $h_0=1$. If $f$ is the $f$-vector of a simplicial complex, this yields precisely the $h$-vector of a simplicial complex \cite{something}. If $X$ is a integral unimodular simplicial complex (as is the case throughout this paper) with $f$-vector $f$, then $h^*$ is the Ehrhart $h^*$-vector or Ehrhart $\delta$-vector of $X$ \cite{something}.\footnote{Note that in Ehrhart theory one does not have to compute the $f$-vector first, but can define the $h^*$-vectors directly as the coefficients of the Ehrhart polynomial in term so the binomial basis $\binom{k+d-i}{d}$ for $i=0,ldots,d$.} These definitions apply also to partial complexes. Note that 
\[ 
h^*_0=\sum_{i=0}^d (-1)^i f_i
\]
from which it follows that $h$- and $h^*$-vectors of a complex coincide if and only if its Euler characteristic is zero.
