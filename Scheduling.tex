...
More formally, decision trees of boolean expressions defined recursively as follows.
...
\]
where $\varphi$ is an inequality of the form $x_i \leq x_j$ and $\psi_t,\psi_f$ are decision trees. As decision trees are binary trees, it will be convenient to distinguish notationally between a node $v$ of the tree and the boolean expression $\Phi_v$ at that node.
\end{definition}

Every leaf $v$ of a decision tree corresponds to a conjunction of inequalities which we call the \emph{cell} at $v$. The cell at $v$ is the conjunction of $\Phi_v$ (the inequalities given by the leaf $v$ itself) and the constraints given in the if-clauses of ancestors of $v$, negated according to whether $v$ resides in the ``true" or ``false" branch of the corresponding if-then-else clause. Let $v$ denote a leaf of the tree and let $v_0,\ldots,v_k$ denote its ancestors. Then $\Phi_{v_i}$ is a boolean formula of the form ``if $\varphi$ then $\psi_t$ else $\psi_f$'' and we define $\varphi_{v_i}' := \varphi$ if $v$ resides in the branch $\psi_t$ and $\varphi_{v_i}' := \neg\varphi$ if $v$ resides in the branch $\psi_f$. We denote by $\rho_v$ the conjunction $\varphi_{v_0}'\wedge\ldots\wedge\varphi_{v_k}'\wedge\Phi_v$ and by $P_v$ the polyhedron of all $x\in \RR^n$ satisfying $\rho_v$. By slight abuse of terminology, we will refer to both $\rho_v$ and $P_v$ as the \emph{cell} at $v$. Note that the set of formulas $\rho_v$ for all leaves $v$ of the tree corresponds to conjunctions in the disjunctive normal form of the decision tree.

\comment{Introduce example from coxeter\_complex.pdf here.}
...
\]
where the disjunction is disjoint and the $C_i$ are nice of dimension $d$, then $\allow(S)$ is partitionable.
\end{theorem}

Because the disjunction is disjoint, this theorem follows immediately, if we can prove it for nice conjunctions.

\begin{lemma}
A nice conjunction $C$ of dimension $d$ is partitionable.
\end{lemma}
...
\begin{theorem}
Let $S$ be a decision tree such that all cells of $S$ are nice of dimension $d$. Then $\allow(S)$ is partitionable. Moreover, disjoint unions of such decision trees are partitionable.
\end{theorem}
...
\comment{Felix' ToDo List
\begin{enumerate}
\item Introduce co-nice decision trees.
\item Expand proof of partitionability.
\item Extend proof to cover the co-nice case.
\item Remark on $k$-partitionability.
\item Add detailed example to text.
\item Redo pictures.
\item Add detailed example from coxeter\_complex.pdf.
\item Find a different word for "nice".
\end{enumerate}
}
