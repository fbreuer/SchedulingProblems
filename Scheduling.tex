...
$\omega:[d]\rightarrow[k]$ such that $\phi(\omega)$ is true.  We
...
$k$ time slots.  Consider three tasks and the following constraints:
...
is in fact a polynomial function in $k$ and includes the chromatic
polynomial, the zeta polynomial of a lattice, and a newly defined
arboricity polynomial all as special instances. The arboricty
polynomial counts the number covers of a matroid with independent sets
or, in a special case, the number of covers of a graph with disjoint
spanning trees.  Many scheduling problems are naturally associated to
...
Geometrically, we consider the space of all solutions via Ehrhart
theory, hyperplanes arrangements, and the Coxeter complex of type A.
As special instances we find the chromatic functions of Sagan and
Gebhard, and Stanley, the chromatic complex of Steingrimmson, the
matroid invariant of Billera-Jia-Reiner, and Ehrenborg's
quasisymmetric function for posets.
...
for stronger results.  If a scheduling problem can be written as a
union of \emph{locally convex decision trees} for example then the
...
positive in the fundamental bases.
...
to the $h$-vector of the \emph{allowed subcomplex} of solutions.
...
subcomplex.  And, Section~\ref{arboricty} introduces  the arboricty polynomial.
...
as $31|4|2$.  We will use ordered set partitions to represent integer
points whose relative ordering of coordinates is given by the blocks
of the partition.  For example, $31|4|2$ represents all integre points
...
polynomial as a specialization of the NCQSym object.
...
\begin{example}
Consider the weight class of points that satisfy $x_1 = x_3 < x_2 =
x_4$.  The corresponding ordered set partition $\Phi$ is $(13|24)$.
$$M_{13|24} = x_1x_2x_1x_2 + x_1x_3x_1x_3 + x_2x_3x_2x_3 + x_3x_4x_3x_4 + \cdots$$  
\end{example}
 ...
 different construction, however. Here we associate to a monomial
 ...
 works because we are working with non-commuting variables here, so
...
start at the same time.  The solutions corresponds to the weight
classes $\{x_1 < x_2\}$ or $\{x_1 = x_2\}$ and are recorded by the
...
$$ \ncS_S = \sum_{\Phi, \Delta(a) =\Phi, a \textrm{satisfies} S} \ncM_{\Phi}. $$
...
set partitions.  
The type map maps elements of NCQSym to Qsym by sending ordered set partitions to compositions.
...
variables to commute.  In this way we extend to the type map to nc-quasisymmetric functions themselves.
...
$$\ncM_{\Phi} ({\bf 1}^k) = { k \choose l(\Phi) },$$
where $l(\Phi)$ is equal to the length, i.e. the number of blocks, of the partition.
...
\comment{Needs short introductory paragraph mainly to give an overview of what is coming in this section.  Connecting NCQSym, Ehrhart and Scheduling.}
...
From this counting interpretation of the coefficients $f_i$ it follows immediately that the $f_i$ are non-negative integers. Moreover there are at most at most $i!\cdot S(n,i)$ ordered set partitions of $[n]$ with $i$ non-empty parts, which proves the upper bound on the $f_i$. 
...
give strict atomic formulas: for all edges $\{i,j\} \in E$, $\omega_i
\neq \omega_j$.  
...
chromatic counting function is usually established as a polynomial
...
specialization of a symmetric function and as an Ehrhart polynomial.  
...
\begin{theorem}
\label{hilbert}
Let $S$ be a scheduling problem. Then the $h$-vector of the shifted scheduling polynomial $\poly_S(k-1)$ is the $h$-vector of the allowed subcomplex $\allow(S)$ and the $h$-vector of the polynomial $(k-2)^n-\poly_S(k-1)$ is the $h$-vector of the forbidden subcomplex $\forb(S)$, i.e.,
\begin{eqnarray*}
...
\section{Positivity, Reciprocity and Partitionability}
\label{geometry}
...
of these expansions to the geometry of the allowed and forbidden complexes.  
...
% Moreover we will require that our decision trees are ``well-behaved" at each node in the tree.
...
\label{fig:tree}
\end{figure}
The collection of points $x \in \mathbb{R}^n$ that satisfy the conditions of a tree form a union of faces of the cell decomposition induced by the Braid arrangement or equivalently a collection of faces of the Coxeter complex $\cox_A$.  In general, this union of faces can be non-convex (as seen in the Figure) or non star-convex.  Below we show that if a scheduling problem can be expressed as the union of decision trees with good local behavior, then the allowable configuration $\allow(S)$ is partitionable which in turn implies positivity of coefficients for the scheduling functions. 

More formally, decision trees are boolean expression defined recursively as follows.
\begin{definition} A \emph{leaf} is a boolean expression that is a conjunction of inequalities of the form $x_i \leq x_j$ or $x_i < x_j$. A \emph{decision tree} is either a leaf, or a boolean expression of the form
\[
  \text{if $\phi$ then $\psi_t$ else $\psi_f$}
\]
where $\phi$ is an inequality of the form $x_i \leq x_j$ and $\psi_t,\psi_f$ are decision trees. 
\end{definition}
Decision trees are therefore binary trees. For every node $v$ in a decision tree, there is a unique path from the root of the tree to $v$. At each node this path chooses either the ``true" or ``false" branch of the corresponding if-then-else clause. Let $v$ be a node in the tree and let $v_0,\ldots,v_k$ be the sequence of its ancestors. For each node $v_i$ in this path we define $\phi_i$ to be the condition of the if-statement of $v_i$ if $v$ resides in the true branch of the node $v_i$, and its negation if $v_i$ resides in the false branch of $v_i$. The \emph{local condition} at $v$ is then the conjunction of all $\phi_i$. 
%The constraints imposed by $v$ only apply to points $x$ that satisfy the local condition.
 Notice that the set of points $x$ satisfying the local condition is always a polyhedron.

\comment{Example}

The decision trees we study will have to be well-behaved in the following sense. Let $v$ be a node in a decision tree corresponding to the statement (if $\phi$ then $\psi_t$ else $\psi_f$). Let $\ell_v$ denote the local condition at $v$ and let $P$ denote the (possibly half-open) polyhedron defined by $\ell_v$. Let $\phi_=$ denote the equality constraint induced by $\phi$, i.e., if $\phi$ is $x_i \leq x_j$ then $\phi_=$ is $x_i = x_j$. A decision tree is \emph{convex} if the following holds for all nodes $v$ of the tree:
\begin{enumerate}
\item The conditions given by the left and right branches of $v$ agree on the hyperplane given by $\phi_=$, where they define a convex solution set. Equivalently, the boolean formulas $(\ell_v \wedge \phi_= \wedge \psi_t)$ and $(\ell_v \wedge \phi_= \wedge \psi_f)$ are equal.
%both equivalent to a formula $\kappa$ which is a conjunction of inequalities.
\item The polytope defined by  $(\ell_v \wedge \phi_= \wedge \psi_f)$ has codimension 1. On the level of boolean functions this means that $\phi_=$ is the only equality implied by  $(\ell_v \wedge \phi_= \wedge \psi_f)$.
\end{enumerate}

Let $\tau$ be a convex decision tree and let $P$ be the set of all $x$ that satisfy $\tau$. Then $P$ is contractible as a topological space.  We say $P$ is open or closed, if all of its boundary is open or closed, respectively.
Again note that $P$ is not necessarily convex and it is not necessarily star shaped.
The structure of a decision tree suffices to prove that a scheduling problem is partitionable.
% Moreover $P$ is a manifold, however, parts of its boundary may be open. 
% (i.e., there does not exist a point that can be connected to all other with a straight line).
%\end{proposition}

%In particular this condition suffices to show partitionability.

\begin{theorem}
\label{thm:tree-part}
If a scheduling problem $S$ can be writen as a disjoint union of convex decision trees, i.e. a disjunction of convex decisions trees such that there is no $x$ that simultaneously satisfies two or more of the decision trees, then $\allow(S)$ is partitionable.
\end{theorem}

%Note that all scheduling problems that can be written as disjunctions of conjunctions are trivially of this form.

The structure of the a decision tree also suffices to prove the following reciprocity theorem.

\begin{theorem}
If $\Phi$ is a scheduling problem that is a disjoint union of open  convex decision trees and $\Phi'$ is the scheduling problem given by the corresponding union of closed  convex decision trees and this union is disjoint, then 
\[
  \xi_\Phi(-k) = (-1)^d \xi_{\Phi'}(k).
\]
\end{theorem}

This theorem can be strengthened to include the case of closed convex decision trees that are not disjoint.

\begin{theorem}
If $\Phi$ is a scheduling problem that is a disjoint union of open  convex decision trees and $\Phi'$ is the scheduling problem given by the corresponding union of closed convex decision trees, then 
\[
  \xi_\Phi(-k) = (-1)^d \sum_{\tau} \xi_{\tau}(k),
\]
where the sum ranges over the closed decision trees $\tau$ in $\Phi'$. In other words, up to sign, $\xi_\Phi(-k)$ counts the lattice points $x\in\Phi'$ with multiplicity given by the number of decision trees $\tau$ the lattice point $x$ is contained in.
\end{theorem}

\comment{fb:
It could be that the decision tree condition is strong enough to show the existence of a convex ear decomposition. However, the standard methods do not suffice for that purpose, as far as I know. We would need additional arguments.
}

\bigskip\hrule\bigskip

\comment{Construction site to get the proofs done.}

\comment{fb: After a closer look at the previous definitions and statement of the partitionability theorem, I had the impression that both the proof and the statement of the theorem would be much simpler using the following setup.}
...
We call a formula $C$ \emph{nice}, if it is equivalent to a conjunction $\bigwedge_j I_{j}$ of inequalities $I_j$ such that at most one of the $I_j$ is strict and all of the $I_j$ are facet-defining. (Note that the $I_j$ are facet-defining if the closure $\bar{C}$ of (the polytope defined by) $C$ has irredundant (!) description $\bar{C} = \bigwedge_j \bar{I}_j$, where $\bar{I}_j$ denotes the weak inequality defined by $I_j$.)

We say a partial polytopal complex is $k$-partitionable, if it can be written as the disjoint union of half-open simplicies of dimension $k$. (In our case, we are of course restricted to use simplicies given by the Braid triangulation.)
...
\]
where the disjunction is disjoint and the $C_i$ are nice of dimension $k$, then $\allow(S)$ is $k$-partitionable.
\end{theorem}

Because the disjunction is disjoint, this theorem follows immediately, if we can prove it for nice conjunctions.

\begin{lemma}
A nice conjunction $C$ of dimension $k$ is $k$-partitionable.
\end{lemma}

\begin{proof} 
(Sketch.) The Braid triangulation gives a regular triangulation of $C$. As $C$ is convex, this triangulation admits a line shelling (of the closure of $C$). Because $C$ is nice, at most one face of $C$ is open. By choosing the line-shelling such that the simplicies incident to the open face of $C$ are "seen first" by the shelling, we can construct a shelling order that induces a partition of (the half-open) $C$.
\end{proof}

Note that, by symmetry, this also works for "co-nice" polytopes that have just one closed face and are otherwise open. \comment{TODO: Check that this really works.}

Now we can apply this tool to decision trees. We use the following definition of a decision tree.

\begin{definition} A \emph{leaf} is a boolean expression that is a conjunction of inequalities of the form $x_i \leq x_j$. A \emph{decision tree} is either a leaf, or a boolean expression of the form
\[
  \text{if $\phi$ then $\psi_t$ else $\psi_f$}
\]
where $\phi$ is an inequality of the form $x_i \leq x_j$ and $\psi_t,\psi_f$ are decision trees. 
...
\comment{fb: In contrast to what we have before, leaves must now be conjunctions of weak inequalities.}

Every leaf of a decision tree defines a cell. Let $v$ denote a leaf of the decision tree $S$. Let $Q_v$ denote the conjunction of weak inequalities corresponding to $v$. Let $R_v$ denote the conjunction of all the conditions in the if-clauses above $v$ in the tree, where the condition is negated if $v$ resides in the "false" subtree at that node. Then the \emph{cell} at $v$ is the polytope defined by $Q_v \wedge R_v$.

\begin{theorem}
Let $S$ be a decision tree such that all cells of $S$ are nice of dimension $k$. Then $\allow(S)$ is $k$-partitionable.
\end{theorem}

\comment{TODO: Examples with pictures. First draft of an example in coxeter\_complex.pdf}
...
\begin{proof} 
(Sketch.) A decision tree is the disjoint union of all its cells. All cells are $k$-partitionable because they are nice. Therefore the decision tree is $k$-partitionable.
\end{proof}

\comment{fb:
\begin{enumerate}
\item The above condition for the partitionability theorem is incomparable with the one that we had before. Still, I think of it as weaker, since it excludes some decision trees that I wanted to cover.
\item However, the above condition has going for it that it includes some scheduling problems $S$ such that $\allow(S)$ has non-trivial topology (an annulus, for example).
\item Most importantly: The proof for this definition is straightforward, whereas I ran in some unexpected trouble with the other definition.
\item Should we allow complexes of codimension $>1$? Should we talk about $k$-partitionability, or restrict to $k=d$? (Contrary to what I wrote before, I do not think this makes a difference wrt star-convexity.)
\item Currently, (if we restrict ourselves to connected allowable subcomplexes) we get non-convex allowable subcomplexes, but not non-star-convex allowable subcomplexes. The reason is that because our allowable subcomplexes are closed (mostly), they always include the main diagonal of the $[0,1]^d$ cube and are thus star-convex. The best way out would be to allow \emph{co-nice} cells. This way we could construct open allowable subcomplexes that can then be properly non-star-convex. Two questions in this regard:
\begin{itemize}
  \item How to extend the proof of partitionability to co-nice conjunctions?
  \item Is it possible to construct a non-star-convex example, even if $k=d$?
\end{itemize}
\item We would need to place some additional restrictions on the decision tree to guarantee that reciprocity works. I am not sure it is worth opening this can of worms for this paper, though. Maybe we should leave reciprocity for the next one?
\end{enumerate}
}
...
of the allowable subcomplex implies positivity of the scheduling
...
The poset of all compositions that refine $\alpha$, ordered by the refinement relation, forms a boolean lattice of dimension $n-l(\alpha)$ where $n$ is the total number of elements in the ground set and $l$ is the length of $\alpha$. For example, the composition $3+1$ is refined by $1+2+1$, $2+1+1$ and $1+1+1+1$, where $1+2+1$ and $2+1+1$ are incomparable under the refinement relation.
...
\[
  L_\alpha = \type(\ncL_{\Phi_\alpha}) \;\;\; \text{ and } \;\;\; L_\alpha(\mathbf{1}^k) = \ncL_{\Phi_\alpha}(\mathbf{1}^k) = \binom{k+n-l}{n}
\]
for any composition $\alpha$, where $\Phi_\alpha$ denotes any ordered set partition with $\operatorname{type}(\Phi_\alpha) = \alpha$, and $l$ is the length of $\alpha$.
...
\]
i.e., $\mathcal{F}$ specializes to $F$ when we allow variables to commute and both $\mathcal{F}$ and $F$ specialize to $p$ when we plug in $k$ ones. Moreover, let $\mu_\Phi$, $\mu_{(\Phi_\text{c};\Phi_\text{f})}$ and $\lambda_\alpha$ denote coefficient vectors of $\mathcal{F}$ and $F$ in terms of the fundamental bases and let $(0,h^*_1,\ldots,h^*_n)$ denote the $h^*$-vector of $p$, i.e.,
\begin{eqnarray*}
...
\begin{theorem}
\label{unique}
Let $S$ be a scheduling problem such that $\allow(S)$ is closed under the directed refinement relation. If for every $\Phi \in \allow(S)$ there exists a \emph{unique} coarsest allowed ordered set partition $\Phi_c \preceq \Phi$ such that $\Phi$ is a directed refinement of $\Phi_c$, then the coefficients of $\SSS_S$ and $Q_S$ in terms of the fundamental bases as well as the $h^*$-vector of $\poly_S$ are non-negative.
\end{theorem}
...
Furthermore,  for any  $\Phi \allow(S)$ there exists a $\Phi_f$ of length $n$ that is a directed refinement of $\Phi$, hence $\Phi$ is contained in the interval with $\Phi_f$ as its maximal element. Therefore, $P$ forms a partition of $\allow(S)$  which completes the proof of the theorem.
\end{proof}
...
$\{x_i = x_j | \, i,j \in E\}$. The graphical zonotope ${P}_G$ is
the zonotope dual to this arrangement formed by the sum of all normals
to all planes in the arrangement.  Geometrically, this leads to a
perspective first noted explicitly by Hersh and Swartz: the coloring
complex is the codimension one skeleton of the normal fan of
${P}_G$ as subdivided by the Coxeter complex~\cite{HS}.

This construction was extended from graphical zonotopes to generalized
permutahedron in~\cite{ABK}.  Generalized permutahedron are typically
...
characterization of generalized permutahedron is the following: A
polytope is a generalized permutahedron if and only if its normal fan
is refined by the Braid arrangement~\cite{PRW, MPSSW}.  Hence the
...
defining the forbidden configuraiton $\forb(S)$ to be the codimension
...
defining $v$.  Hence the valid $\omega$ schedules correspond to all
integer points in the interior of the normal fan.  In~\cite{ABK}, it
is shown that the scheduling nc-quasisymmetric function is $\ncL$
positive and $\forb(S)$ is $h$-positive.

In~\cite{HS}, this perspective of the normal fan is used to prove that the coloring
complex has a convex ear decomposition which implies strong relations on
the chromatic polynomial. The authors attempt to generalize this result from chromatic polynomials of graphs to characteristic polynomials of matroids.
...
polynomials to chracteristic polynomials, but from the scheduling
...
The corresponding scheduling problem $S$ does satisfy the conditions of Theorem~\ref{unique}, but can not be written as the conjunction of disjunctions.
...
\subsection{Allow is a subcomplex, the Co-fundamental basis and the $\ncN$-coefficients}
...
%opposed to simply a collection of faces). 
 Here we consider Scheduling problem such that $\allow(S)$ is a valid
 ...
 behavior.  The co-fundamental basis for quasisymmetric functions is
 defined analogously to the fundamental basis but instead of summing
 over all compositions that refine a fixed composition, one sums over
 all compositions that coarsen a fixed composition:
...
to the collection of all flags of a graded poset, then clearly $\allow(S)$ is closed under coarsening.  In this case, the
quasisymmetric function QSym(S) has a particularly nice expansion.
...
in the monomial basis, the quasisymmetric function QSym(M) is indexed
by compositions recording the size of each step in the flag.  This
...
poset and recording the rank jump of the flag at each step.  Although
...
manipulation of the Mobius function, and the compositions indexing the
...
Consider $\omega$ as a weight function on the ground set of $M$ and define the weight of a basis $B$ to be: $\omega(B) = \sum_{e\in B} \omega(e)$.   Furthermore define $M_{\omega}$ to be the induced matroid of all
$\omega$-minimal bases. 
...
The function $\omega$ is called $M$-loopless if the matroid
  ...
  Bergman fan of $M$.  Considering $\omega$ simply as an integer
  point, we associate a flag $\mathcal{F}_{\omega}$ as above.
In ~\cite{AK} it is shown that a weight function $\omega$ is $M$-loopless (equiv. in the Bergman fan)
...
The Bergman Scheduling problem is the scheduling problem with allowable configuration $\allow(S)$ equal to all $\Delta(\om)$ such that $\mathcal{F}_{\omega}$ is a flag of flats of $M$. 
...
%% \subsection{Extended example: QBergman}
...
flats.  The contribution to QSym(S) will be $M_{(1,3)}$ since $(1,3)$
...
\end{example}
%% Consider any $\omega \in \mathbb{N}^n$ such that
...
nc-quasisymmetric function are closed under {\em coarsening} and moreover form the collection of flags of the lattice of flats of the matroid. 
...
\item Up to sign, $ [N_{|E|}]$ is equal to the Mobius function of the lattice
of flats $\mu(0,1)$, equivalently the reduced Euler characteristic of the order complex of
...
all bases componentwise less than or equal to $(135)$.  The Bergman quasisymmetric function is given by
\begin{align*}
& QSym(M_{<135>})  \\
...
For $n=3$,  consider the Braid arrangement in $R^3$, the ground set space of the
...
intersection: $x_1 = x_2 = x_3$. Hence the corresponding element of NCQSym is 
...
In general,
$$A_{M(C_n)}(k) = \sum_{m=2}^{n} S(n,m) \cdot k(k-1) \cdots (k-m+1), $$
where $S(n,m)$ is the stirling number of the second kind. 
Using the identity that $\sum_0^n S(n,m) \cdot k(k-1) \cdots (k-m+1) = k^n$, we find that the number of covers of the cycle graph with at most $k$ forests is
