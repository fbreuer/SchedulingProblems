...
Note that, by symmetry, this also works for "co-nice" polytopes that have just one closed face and are otherwise open.
...
\comment{TODO: Examples with pictures. (I hope to digitize a nice blackboard example that I have as soon as possible.)}
...
\item If we allow $k<d$, this definition does include non-convex, non-star-convex cases. But if $k<d$, the meaning of partitionability is not so nice. We would have to use a shifted basis to get non-negativity.
\item On the other hand, if we restrict to $k=d$, then I do not know whether we can avoid star-convexity with the decision trees. (How to get rid of the main diagonal, given that our decision trees are now (mostly) closed?)
